...
Lastly, before we try running our system again, let's change our connection string in `system.edn`. We can drop the `:test` profile, and use a `:default` one with the following value.
